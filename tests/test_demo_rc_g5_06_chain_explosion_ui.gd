...
func _test_readiness_cancel_trigger_and_redaction() -> void:
	...
	var status: Label = view.find_child("ChainExplosionStatus", true, false); var applied: Label = view.find_child("ChainExplosionEffectsApplied", true, false); var locked: Button = view.find_child("ChainExplosionLocked", true, false)
	...
	_ok(applied != null and applied.text.contains("후속 효과 적용 완료") and applied.text.contains("G8-01"), "combat effects apply while victory remains pending for G8-01")
