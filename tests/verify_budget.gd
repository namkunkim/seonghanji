...
func _init() -> void:
	...
	#
	# ⚠ **두 경로가 어긋난다.** `verify_power.gd` 는 문서의 동원율 상수(조조 0.35)를
	#   그대로 곱해 74 를 얻지만, `Faction.mobilized` 는 §3.4-b 산식으로 계산한다.
	#   **M0 결손 ④ 「동원율 — 세는 방식 판정 필요」가 여기서 눈에 보인다.**
	print("실동원 — 산식 대 문서 (M0 결손 ④)")
	print("  %-8s %6s %6s   %6s %6s   %s" % [
		"세력", "국력", "산식", "동원율", "문서", "전대"])
	var docrate := {"조조": 0.35, "유장": 0.60, "손권": 0.75, "유종": 0.60}
