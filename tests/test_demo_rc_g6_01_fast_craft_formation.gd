...
func _test_historical_setup_and_catalog() -> void:
	...
	_eq(catalog.penalty_application, {"mobility_percent":"active_g4_movement", "accuracy_percent":"pending_battle_consumer", "formation_change_percent":"pending_battle_consumer"}, "penalty consumer capability is explicit")
	_eq(catalog.out_of_scope, ["in_battle_equipment_change", "tactical_mission_change", "fuel", "ammo_resupply", "automatic_return", "drift", "rescue_result", "capture_result"], "later G6 systems remain exactly out of scope")
...
func _test_overcap_curve_and_invalid_boundaries() -> void:
	...
	_eq(metric.pending_penalties, ["accuracy_percent", "formation_change_percent"], "unconsumed accuracy and formation-change penalties are disclosed as pending")
