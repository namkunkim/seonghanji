...
func _build_cost_panel(summary: Dictionary) -> Control:
	...
	var warning := Label.new(); warning.name = "FastCraftBudgetWarning"; warning.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; warning.text = "초과 편성은 적용 가능합니다. 기동 불이익은 기존 G4 이동에 적용되며, 명중·진형변경은 준비 단계 예정치로 아직 전투에 소비되지 않습니다." if bool(summary.get("over_cap_warning", false)) else "권장 지휘 비용 이내입니다."; stack.add_child(warning)
	return panel
