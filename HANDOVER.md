...
> 갱신일: 2026-08-23 (J2 검수 완료 반영)
>
> **[핸드오버 요약]** A2·A3(J1) 완료 후 J2 검수 완료 — 일반 무장 249인(원 252인에서 중복 3인 제거)·이역 93인 스탯·성향 확정. 서방 원수급 19인은 확장 콘텐츠로 유보. `officers-256.md`에서 ①명장과 중복·충돌하던 미방·부첨·주태 스탯을 제거하고 ①을 정본으로 정리(`docs/DECISIONS.md` V-09). 다음은 J3·J4·J8(병렬 가능) — `JOBS.md` 참조.
...
| **문서** | 25개 (약 47만 자) + HTML 지도 1개 |
| **코드** | **미착수** |
| **남은 것** | 수치화 · 물량 작성 · 프로토타입 (표기 일괄 개정은 완료) |
...
| `05-narrative/epilogues.md` | 엔딩 후일담 10종 | **표기 개정 적용** |
...
```
~~A2·A3 (스탯 완성)~~  →  ~~J2 (검수)~~  →  J3·J4·J8 (병렬 가능)  →  ...
```

B6(표기 통일)는 완료. A2·A3(스탯, J1)은 2026-08-22 완료, J2(검수)는 2026-08-23 완료 —
다음은 `JOBS.md` §1 큐를 따른다. J3(A6 동원율·함종 비용)·J4(검토포인트 일괄 해소)·J8(B1 시작 배치표)이
서로 독립이라 동시 착수 가능.
단, A2·A3은 **규칙 기반 자동 산출**이라 개별 인물의 서사적 개성까지는 못 담는다 — J2에서 1차 표본 검수는
마쳤으나 전수 검토는 아니므로, 두드러진 인물은 이후 작업(B2 등)에서도 발견 시 손으로 다듬을 것.
...
| `function-events` §9-2 | F-16 조건이 「절의 또는 야심」. 실무형만 있으면 북벌이 영영 미발동 |
| `function-events` §9-4 | F-20 비인접 침투가 등애 전용. 사망 시 영구 미발동 |
| `scenario-219` §6-7 | **육손 부재 시 이릉 원정 성공** → 삼국 구도 붕괴. 세계 상태 재판정 필요 |
| `scenario-228-263` §9-1 | 공세 6회 반복의 피로. 회차별 변주 필요 |
| `endings` §8-1 | Division 엔딩의 「안정 유지」가 대기 플레이가 되면 지루함 |
...
> 설계는 사실상 끝났고, 표기 통일(B6)도 완료됐습니다. 남은 것은 수치·물량입니다.
> 권장 순서는 스탯 완성(A2·A3) → 시작 배치표(B1) → 임계·산식(A4·A5)입니다.
