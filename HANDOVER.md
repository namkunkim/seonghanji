...
> (P0-01·P0-02·P0-03·P0-05·P0-06 ✅ · P0-04·P0-07·P0-08 🟨 · 다음은 P0-09).
> P0-07 은 부분 진행 — 시각 가이드 v1.0 후보, 초상 비의존 쟁점 6건 확정(`114e647`).
> P0-08 은 부분 진행 — `preproduction-license-check.md` 신설(대장 라이선스 전사·통합 뷰 ·
> GATE-PRE-07 5조건 매핑 · 확정 5행)(`ee701a5`). 잔여: 초상 파일별(P0-04/P-02) · 영상 서비스 약관 5항(S6.1) · 스토어 AI 고지란.
...
| **문서** | **41개** + HTML 지도 1개 |
| **코드** | **코어 27파일 · 화면 15파일 · 시험 33섹션 617단언 · 검산기 3종** |
...
```
godot --headless --path . --script tests/run_tests.gd      33섹션 617단언
...
- ~~아트·사운드 방향~~ — **명세 완료.** `asset-ledger.md`(물량) + `ai-media-pipeline.md`
  ...
  **남은 것은 전부 GPU 실행 패스** — `ai-media-pipeline.md` 미작성 「남은 것 6」:
  ① 화풍 기준선 시험생성(모델·SHA-256 + 검토 3) ② 초상 131장 ③ 검토 5 통념 12명
  ④ BGM 6트랙 ⑤ 크레딧 문안 확정 ⑥ 배경·컷씬(S6)
  ...
  - **시각 방향 P0-07 🟨 부분 진행 (2026-08-30 · `114e647`)** — `visual-direction-guide.md` 를
    초안 v0.1 → **v1.0 후보**로. §7 시각 쟁점 10건 중 초상·3D 표본 비의존 **6건 확정**
    (기저 항로 선형 · 세력 색 범위 · 전투 3D 비중 2/3+1/3 · 구지 황폐 수준 · 금색 의미 · 아이콘
    기호 폴백). §8 「검수 체크리스트」를 **품질 판정 기준(통과/재작업)**으로 승격. **잔여 4건**
    (함선 세력 차별화 · 기동병기 실루엣 · 적벽 분기 · 장르 비유 교체)은 P0-04 초상 화풍 방향
    판정 후 · `preproduction-av-plan.md` D3·D5.
  - **라이선스 점검 P0-08 🟨 부분 진행 (2026-08-30 · `ee701a5`)** — `preproduction-license-check.md`
    ...
    Noto). **잔여:** 초상 파일별(P0-04/P-02) · 영상 서비스 약관 5항 원문 확인(S6.1 · D6) ·
    스토어 AI 고지란 형식·수위 · 음원 검토 3 의 `DECISIONS.md` V-항목 신설.
...
>
> 프리프로덕션은 P0-01·P0-02·P0-03·P0-05·P0-06 이 닫혔고 **P0-04(이미지)·P0-07(시각 기준)·
> P0-08(라이선스) 진행 중**, P0-09 가 남았습니다. P0-07 은 초상 비의존 시각 쟁점 6건을 확정했고
> 잔여 4건은 P0-04 뒤입니다. P0-08 은 대장 라이선스를 `preproduction-license-check.md` 한 표로
> 전사·통합했고(확정 5행 · GATE-PRE-07 5조건 매핑), 잔여는 초상 파일별(P0-04) · 영상 서비스
> 약관(S6.1) · 스토어 AI 고지란입니다. 「영상」 정의(P0-06 D1)는 2026-08-30 발주자가
> **C(혼합)**으로 확정했습니다.
>
