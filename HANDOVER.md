...
> **2026-09-10 MGMT-HANDOVER-01 — 성한지 프로젝트 진행·일정·작업 프롬프트 관리 — 🟨 진행:** 1차 목표는 적벽 시나리오를 선택하고 손권 함대를 직접 집결시켜 손유 측 5페이즈 전투를 결착한 뒤 승패·판세·후일담 결과를 확인하는 단일 완주 흐름이다. `demo-rc-completion-review-checklist.md`에 해야 할 일, 하지 말아야 할 일, P1-01~P1-10 완료 판정을 추가했다. `main`/`origin/main`은 `111d0f2`로 일치하나 수정 17항목·미추적 276항목이 남아 G1은 미수용이다. 다음 구현 작업은 다른 변경을 보존한 별도 통합 후보에서 핵심 흐름에 필요한 소스·시험·필수 자산만 고정해야 한다. 전체 캠페인·추가 시나리오·신규 실시간 3D·대규모 자산·CI 확장은 1차 목표 뒤로 둔다.
