...
> **GATE-PRE-02 정본·권위 계약 = 계약 A**(**V-60**) · **GATE-PRE-04 QA 전략 = 계약 A**(**V-61**,
> 밸런스 기준선은 `Q-01` 재현율 40~60% 복귀 완료 시 잠금) · **GATE-PRE-03 캠페인 저장 요구사항
> = 초안대로 승인**(**V-62** · `preproduction-save-contract.md` — 저장 = 시드+명령 로그, 나머지
> 파생 / 경계 스냅숏 / `A-01`·`A-02` 완료 기준).
> `preproduction-dev-gate.md` §8 게이트 승인본 초안 · **GATE-PRE 충족도 ✅ 6/8**.
> 남은 발주자·검수자 닫힘 판정 = **GATE-PRE-05**(측정 항목)·**07**(승인 스냅숏).
...
> `preproduction-save-contract.md` — 저장 = 시드+명령 로그, 세력·함대·경제·외교·이벤트·전투
> 결과는 파생 / 경계 스냅숏 / 손상 복구 A-02 전 로드 거부 / 자동 저장 월 정산 / §2·§3·§4.2 =
> `A-01`·`A-02` 완료 기준) · **GATE-PRE-04 QA 전략 = 계약 A (V-61** — 밸런스 기준선은 `Q-01`
> 완료 시 잠금, 그때까지 M0 60.0% ↔ 현재 67.0% 분리) · L3(D6-1) 채택(V-52) · D6-2(V-54).
>
> **GATE-PRE 충족도 ✅ 6/8** (01·02·03·04·06·08). 결정 대기 큐 열림 **17항**.
>
> **다음 발주자 질문 (한 번에 하나씩):**
> **GATE-PRE-05 (시간·수익 원칙 — 측정 항목)** — 전반부(유료 시간 단축 대안 제외)는 불가침
> §2로 이미 닫힘. 남은 것은 **무료 배속 상한 · 편성 중 자동 강등 · 전투 개입 시 정지 여부를
> 어떤 실기 지표로 판정할지**의 정의(최종값이 아니라 측정 항목 목록). 최종값은 `Q-04` 실기
> 관찰. 그다음 GATE-PRE-07(승인 스냅숏) · D3 조달 순서 · D7 · DEC-04~07.
