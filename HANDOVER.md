...
> 밸런스 기준선은 `Q-01` 재현율 40~60% 복귀 완료 시 잠금).
> `preproduction-dev-gate.md` §8 게이트 승인본 초안 · **GATE-PRE 충족도 ✅ 5/8**.
> 남은 발주자·검수자 닫힘 판정 = **GATE-PRE-03**(저장 계약 초안 `preproduction-save-contract.md`
> 승인 대기, 별도 세션)·**05**(측정 항목)·**07**(승인 스냅숏).
...
>
> 발주자 결정 상황: 「영상」 정의(D1) = C 혼합 · **L2 3D 증거창(D2) = 채택(V-55)** ·
> **D4·D5 = 무장 위성 시스템 신설 · 전투기 설계 선행 (V-58)** · **G-08·G-09 = 첫 프로토타입
> 범위 밖 (V-59)** · **GATE-PRE-02 정본·권위 계약 = 계약 A (V-60)** ·
> **GATE-PRE-04 QA 전략 = 계약 A (V-61)** — 결정성 계약 + 시험 5층 + 실행 신뢰성,
> **밸런스 기준선은 `Q-01`(재현율 40~60% 복귀) 완료 시점에 잠금**(그때까지 M0 60.0% ↔
> 현재 67.0% 분리) · L3 컷씬(D6-1) = 채택(V-52) · D6-2 = Runway 우선·Wan 2.2 폴백(V-54).
>
> **GATE-PRE 충족도 ✅ 5/8** (01·02·04·06·08). 결정 대기 큐 열림 **18항**.
>
> **다음 발주자 질문 (한 번에 하나씩):**
> **GATE-PRE-03 (캠페인 저장 요구사항)** — 초안 `preproduction-save-contract.md`(별도 세션)가
> 서 있고 **발주자 승인 대기**. 저장 대상 8도메인 포함/제외/파생 · 스냅숏/로그 경계 · 규칙
> 버전 불일치 처리 · 손상 복구 · 지문 인수 조건 7종. 요구사항 승인만으로 게이트 통과, 구현은
> `A-01`·`A-02`·`C-03` 이월. 그다음 GATE-PRE-05(측정 항목)·07(승인 스냅숏) · D3 조달 순서 ·
> D7 · DEC-04~07.
