...
> **P0-09** — 발주자 결정: D2 = **L2 3D 증거창 채택**(`DECISIONS.md` **V-55**) · D4 = **무장 위성
> 방어 시스템 신설** · D5 = **전투기 설계 선행**(**V-58**) · G-08·G-09 = **첫 프로토타입 범위 밖**
> (**V-59**, 둘 다 후속) · **GATE-PRE-02 정본·권위 계약 = 계약 A 확정**(**V-60**).
> `preproduction-dev-gate.md` §8 게이트 승인본 초안 · GATE-PRE 충족도 ✅ 4/8.
> 남은 발주자·검수자 닫힘 판정 = **GATE-PRE-03·04·05·07**.
...
> **D4 무장 위성 방어 = ㉯ 시스템 신설 · D5 전투기 = ㉮ 설계 선행 (V-58)** ·
> **G-08·G-09는 첫 프로토타입 범위 밖 = 둘 다 후속 (V-59)** ·
> **GATE-PRE-02(정본·권위 계약) = 계약 A 확정 (V-60)** — 역할 계약
> `docs=근거/data=값/core=판정/UI=표시` + 화면↔코어 갈라짐 5건 전수 목록 + 정본 소유자,
> 실이동은 `A-03~A-06` · L3 컷씬(D6-1) = 채택(V-52) · D6-2 = Runway 우선·Wan 2.2 폴백(V-54).
>
> **GATE-PRE 충족도 ✅ 4/8** (01·02·06·08). 결정 대기 큐 열림 **19항**.
>
> **다음 발주자 질문 (한 번에 하나씩):**
> **GATE-PRE-04 (QA 전략)** — 가상 시계·고정 시드군·임의 상태 주입의 공통 시험 계약 · 시험
> 층 정의(단위/100회 캠페인/검산/저장 복원/실기 스모크) · **버전별 밸런스 기준선 보존 방식**
> (M0 통과 60.0% ↔ 계략 배선 후 67.0% 드리프트). 그다음 GATE-PRE-03(저장)·05(측정 항목)·
> 07(승인 스냅숏) · D3 조달 순서 · D7 · DEC-04~07.
