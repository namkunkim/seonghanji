...
## 0.1 M0 현황 — 2026-08-28

```
godot --headless --path . --script tests/run_tests.gd      32섹션 375단언
godot --headless --path . --script tests/run_campaign.gd   100회 · 합격 3/3
...
| 지표 (`ai-design.md` §11.1) | 목표 | 최종 | |
|---|---|---|---|
| 역사 재현율 | 40~60% | **60.0%** | ✅ |
| 조기 종료율 | 20% 이하 | **0%** | ✅ |
| 세력 승률 편차 (**주역 셋** · V-40) | 3배 이내 | **1.5배** | ✅ |
| 미발동 이벤트 | 0종 | **1종** (F-10) | ❌ |

**M0 결손 6종 중 다섯이 닫혔다.** 남은 것은 ⑥ 의 **계략**뿐이다.
...
| | 무엇 | 근거 |
|---|---|---|
| **계략** | M0 결손 ⑥ 의 마지막. `combat.md` §5 에 산식이 **전부 확정**되어 있고 코드가 안 읽는다 | §5.3~§5.6 |
