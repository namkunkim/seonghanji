...
>
> **큐 종료 후 정리(2026-08-23):** 두 건을 닫았다. ① **사기 범위** — `function-events.md` 0~100 대 `combat.md` 45~125의 어긋남을 **게이지 0~125 · 명목 100**으로 확정하고 「고양」 구간을 신설. ② **이릉협도 화공 ×3** — 계수를 낮추지 않고 **연영도**를 신설해 조건부로 전환(`DECISIONS.md` V-20).
...
**2026-08-23에 닫힌 것** — `combat` §10-12(**이릉협도 화공 ×3이 단일 판정으로 전투를 끝낸다**)는
**연영도(Encampment Sprawl) 신설로 해소했다**(`combat.md` §5.5-b · `DECISIONS.md` V-20).
계수 ×3.0은 낮추지 않고 **공짜가 아니게** 했다 — 협도 진입이 아니라 **너머에서 반년을 못 이겼을 때** 열리며,
그 시계는 방어 지휘관이 돌린다(육손 ×2.0). **연영도가 피해를, 방어 지휘관이 발동 여부를 정한다.**
...
>
> 다만 코드로 넘어가기 전에 정리할 것이 §5에 남아 있습니다. 그중 하나는
> **밸런스가 아니라 설계 충돌**이라 먼저 판단이 필요합니다 —
> `world-state-setup` §10-1(**219 일극형이 임계 2.0에 미달**).
> 임계를 낮추면 적벽 판정이 위태롭고, 독립 세력을 지우면 형의 성격이 사라집니다.
>
> **프로토타입부터 갈지, 이것을 먼저 닫을지** 정해 주십시오.
