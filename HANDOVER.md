...
> 갱신일: **2026-09-01** — **M0 통과 · S3 클라이언트 진행 중 (S3.2~S3.4·S3.6 ✅).**
>
...
> (P0-01~P0-08 ✅ · **P0-09 진행 중**).
> P0-08 완료(2026-09-01) — `preproduction-license-check.md` 로 대장 라이선스 전사·통합,
> V-50~V-54 반영(공용 11종 SHA·라이선스 잠금 · SFX AI 배제 · L3·스토어 고지 · 게임 내
> 크레딧 4구획 · D6-2 Runway 우선·Wan 2.2 폴백). Runway 공식 약관 5항은 S6.1 후속.
> **P0-09** — 발주자 결정: D2 = **L2 3D 증거창 채택**(**V-55**) · D4·D5 = **무장 위성 시스템 신설 ·
> 전투기 설계 선행**(**V-58**) · G-08·G-09 = **첫 프로토타입 범위 밖**(**V-59**) ·
> **GATE-PRE-02 = 계약 A**(**V-60**) · **GATE-PRE-03 캠페인 저장 = 초안대로 승인**(**V-62** ·
> `preproduction-save-contract.md` — 저장 = 시드+명령 로그, 나머지 파생 / 경계 스냅숏 /
> `A-01`·`A-02` 완료 기준) · **GATE-PRE-04 QA 전략 = 계약 A**(**V-61**, 밸런스 기준선은 `Q-01`
> 완료 시 잠금) · **GATE-PRE-05 시간·수익 원칙 = 선택 A**(**V-63** — `Q-04` 측정 지표 확정 +
> 잠정 기본 = 현 구현: 배속 상한 ×4 · 편성 중 자동 강등 없음 · 전투 개입 시 일시정지 없음) ·
> **GATE-PRE-07 라이선스 = 승인 스냅숏 §8.4 승인**(**V-64** — P0-08 V-50~V-54를 최소 기록
> 6항으로 고정).
> **개발 착수 게이트 8개 계약이 전부 ✅** (`preproduction-dev-gate.md` §8).
> 필수 콘텐츠 범위표는 **`단기판 필수 범위 v1.0`** 으로 버전 고정 (§8.5 초안 · 보류 목록 11항
> · 상향 트리거). **P0-09 최종 승인의 문서 작업은 끝났다** — 잔여는 최소 기록 3(밸런스 기준선
> 실제 잠금 = `Q-01` 완료 후, 방식은 V-61 확정)뿐이며 개발 착수 후 실행이다.
> Godot 코드는 참고 프로토타입이며 이 단계의 최우선이 아니다.
>
...
> 그 위에 **S3 클라이언트**를 올리는 중이다 — 게임 루프·성역 뷰(SC-L2)·세부 뷰(SC-L3)·
> 함대 편성 UI(SC-F1~F3)가 섰고, 남은 것은 명령 메뉴(S3.5)·배너(S3.7)·저장(S3.8).
...
| **M0** | 게이트 **3/3 통과**(2026-08-28). 계략 배선 후 재측정(08-30) — 재현율 **67.0%**(목표 40~60% 상단 초과) · 조기 종료 **0.0%** · 편차 **2.1배** · F-10 미발동. 되돌릴 대상은 계략·전투 밸런스이며 게이트 재개방이 아니다 (§0.1 · `DECISIONS.md` V-47) |
| **남은 것** | **S3 화면 나머지** — S3.5 명령 메뉴 · S3.7 배너 · S3.8 저장 (§4.1). S3.2~S3.4·S3.6 ✅ |
...
> **프리프로덕션 P0-01~P0-08 이 모두 닫혔습니다.** P0-09(개발 착수 게이트 정의)가 진행 중이며,
> `preproduction-dev-gate.md` §8 에 GATE-PRE-01~08 별 게이트 승인본 초안이 섰습니다.
>
> 발주자 결정 상황: D1 = C 혼합 · **D2 L2 3D 증거창 = 채택(V-55)** · **D4·D5 = 무장 위성
> 시스템 신설 · 전투기 설계 선행 (V-58)** · **G-08·G-09 = 첫 프로토타입 범위 밖 (V-59)** ·
> **GATE-PRE-02 = 계약 A (V-60)** · **GATE-PRE-03 캠페인 저장 = 초안대로 승인 (V-62** ·
> `preproduction-save-contract.md`) · **GATE-PRE-04 QA 전략 = 계약 A (V-61** — 밸런스 기준선은
> `Q-01` 완료 시 잠금) · **GATE-PRE-05 시간·수익 = 선택 A (V-63** — `Q-04` 측정 지표 + 잠정
> 기본 = 현 구현: 배속 상한 ×4·편성 중 자동 강등 없음·전투 개입 시 일시정지 없음) ·
> **GATE-PRE-07 라이선스 = 승인 스냅숏 §8.4 승인 (V-64** — P0-08 V-50~V-54 를 최소 기록 6항
> 으로 고정) · L3(D6-1) 채택(V-52) · D6-2(V-54).
>
> **개발 착수 게이트 8개 계약이 전부 ✅** (01 V-48효과·02 V-60·03 V-62·04 V-61·05 V-63·
> 06 조건충족·07 V-64·08 V-49). 결정 대기 큐 열림 **15항**.
>
> **P0-09 최종 승인의 문서 작업이 끝났다.** 최소 기록 7항 중 ✅ 5 (1·2·4·6·7) / 🟨 2 (3·5).
> ① 최소 기록 1 = 필수 콘텐츠 범위표 **버전 고정 완료** — `단기판 필수 범위 v1.0` (§8.5 초안 ·
> `preproduction-scope.md` §7 정본). ② 최소 기록 3 = 밸런스 기준선 **실제 잠금** = `Q-01`
> (재현율 40~60% 복귀) 완료 후 — 방식은 V-61 확정, **개발 착수 후 실행**. ③ 병렬 항목(게이트
> 승인의 필수 입력 아님): D3 조달 순서 · D7 · 시각 쟁점 4·5·7·8 · DEC-04~07.
> **P0-09 는 이제 발주자의 개발 착수 최종 승인만 남았다.**
