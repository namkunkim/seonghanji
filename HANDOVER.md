...
> (P0-01~P0-07 ✅ · P0-08 🟨 · 다음은 P0-09).
> P0-04는 V-50으로 v2 스타일·FLUX schnell·공용 11종을 발주자 승인했으며, `SC-F3` 실제
> 320px 프레임 연결도 통과했다(`fd521c5`). P0-07은 이 검수로 완료됐다.
> P0-08 은 부분 진행 — `preproduction-license-check.md` 신설(대장 라이선스 전사·통합 뷰 ·
> GATE-PRE-07 5조건 매핑 · 확정 5행)(`ee701a5`). 공용 11종 파일별 SHA·라이선스는 잠금.
> 잔여: 영상 서비스 약관 5항(S6.1) · 스토어 AI 고지란 · 크레딧 문안 확정.
...
5. docs/DECISIONS.md    ← 기각된 안과 그 이유 (V-01~V-48)
...
| `07-production/preproduction-ship3d-procurement.md` | **D3 함선 3D 조달·시각 기준** — 함급 6종 요구사항(역할·실루엣·스케일·LOD·머티리얼·금지) · 4계열 도장·문장·색상 규칙 · CC0 탐색 기준 + 라이선스 검수 양식 · 5일 실패 시 외주·유료 전환 · S5.1 선행 + P0-09 결정 초안 | **신규(2026-09-01)** · L2 채택 가정 조건부 |
...
- ~~아트·사운드 방향~~ — **명세 완료.** `asset-ledger.md`(물량) + `ai-media-pipeline.md`
  ...
  **P0-04 공용 초상 기준선은 실행 완료** — V-50의 FLUX schnell·v2 스타일 공용 11종이
  ...
  - **시각 방향 P0-07 ✅ 완료 (2026-09-01 · `fd521c5`)** — `visual-direction-guide.md`를
    ...
    결정(`preproduction-av-plan.md` D3·D5)으로 남는다.
  - **라이선스 점검 P0-08 🟨 부분 진행 (2026-08-30 · `ee701a5`)** — `preproduction-license-check.md`
    신설: `asset-ledger.md` 라이선스 정본을 한 표로 전사·통합(모델 3 · 서비스 RunPod ToS ·
    서체 Noto Sans KR OFL · CC0군 · AI 영상). 고지 요건 2갈래 분리(게임 내 크레딧 = `ai-media-pipeline.md`
    §8 / 스토어 AI 고지란 = `dev-requirements.md` §7.3 · D6). **GATE-PRE-07 5조건 매핑** — 3(AI 고지·
    크레딧)만 부분 충족, 게이트 닫힘 판정은 P0-09. 확정 5행(FLUX schnell · SDXL · ACE-Step · RunPod ·
    Noto). **공용 11종 파일별 SHA·라이선스는 잠금. 잔여:** 영상 서비스 약관 5항 원문 확인
    (S6.1 · D6) · 스토어 AI 고지란 형식·수위 · 크레딧 문안 · 음원 검토 3의
    `DECISIONS.md` V-항목 신설.
...
> **M0 게이트가 통과했습니다** — 재현율 60.0% · 조기 종료 0% · 편차 1.5배.
> 코어 26파일이 돌고 화면이 하나 섰습니다.
> **다음은 S3 클라이언트**이고 화면 명세는 `docs/06-tech/screens.md` 에 이미 있습니다.
>
> 프리프로덕션은 **P0-01~P0-07이 닫혔고 P0-08(라이선스)만 진행 중**, P0-09가 남았습니다.
> P0-04 공용 11종은 V-50으로 잠겼고 실제 `SC-F3` 320px 프레임 연결·리소스·회귀 검수도
> 통과했습니다(`fd521c5`). P0-08은 대장 라이선스를 `preproduction-license-check.md` 한 표로
> 전사·통합했고(확정 5행 · GATE-PRE-07 5조건 매핑), 공용 11종 파일별 SHA·라이선스도 잠갔습니다.
> 잔여는 영상 서비스 약관(S6.1) · 스토어 AI 고지란 · 크레딧 문안입니다. 「영상」 정의(P0-06 D1)는 2026-08-30 발주자가
> **C(혼합)**으로 확정했습니다.
>
> 남은 발주자 판정은 **P0-06 D2~D7** — L2/L3 채택 여부 · 함선 3D 조달(R-16) ·
> 무장 위성·전투기 규칙(R-17) — 로 **P0-09 / 27주차 잔여**입니다.
> **어느 것부터 보시겠습니까?**
