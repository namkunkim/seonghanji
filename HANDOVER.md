...
> **P0-09 최종 승인 잔여 = 게이트 밖:** 최소 기록 1(필수 콘텐츠 범위표 버전 고정 — 문서
> 작업)·3(밸런스 기준선 실제 잠금 = `Q-01` 완료 후, 방식은 V-61 확정).
...
>
> **다음 — P0-09 최종 승인의 잔여 (게이트 계약 밖):**
> ① 최소 기록 1 = **필수 콘텐츠 범위표 버전 고정** (`preproduction-scope.md` §7 을 버전
> 고정 — 문서 작업). ② 최소 기록 3 = **밸런스 기준선 실제 잠금** = `Q-01`(재현율 40~60%
> 복귀) 완료 후 (방식은 GATE-PRE-04 V-61 확정, 개발 착수 후 실행). ③ 병렬 항목(게이트
