...
> **2026-09-08 DEMO-RC-LT-01 — 적벽대전 5페이즈 전투 및 플레이어블 데모 — PARTIAL:** `Main`의 `적벽` 버튼은 seed 20803의 실제 `Campaign.scenario_03` 명령 경로로 canonical active battle을 만든다. `ActiveBattle`/Campaign은 5페이즈·손실·사기·계략·player command·AI 위임·결과 함대 투영·stable result news·log replay를 담당한다. `RedCliffBattleView`는 레퍼런스 기반 premium HUD(5단계 timeline, 양측 사기 카드, 궤도·교전권·곡선 항로·쐐기 전열의 2/3 전략 지도, 전용 시네마틱 자산과 LIVE 상태 overlay의 1/3 전장, 대형 전술 버튼)이며 판정 수식은 갖지 않는다. GUI executable은 제품 viewport를 1600×900으로 렌더해 6개 캡처를 생성했고, writable `--user-data-dir`에서 file save/restore는 73/0이다. 다만 Computer Use가 Godot native-window의 OS physical input surface를 제공하지 않아 최종 물리 입력 수용만 PARTIAL이다. 다음 단일 Task는 **DEMO-RC-QA-02 — 적벽대전 Windows 1600×900 실제 GPU 플레이·캡처 수용**이다.
