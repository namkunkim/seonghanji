...
목표 미달은 남는다」고 기록했다(`combat.md` §10 검토 14·16·17). **S3.2~S3.6 클라이언트
...
| | 무엇 | 근거 |
|---|---|---|
| ~~**계략**~~ | **배선됨 (`08fe115`, 2026-08-30).** `core/combat/scheme.gd` 신설 · `_resolve_battle` 5페이즈에 연결. 적벽 네 갈래·이릉 12값이 문서와 일치(`verify_chibi`). **다만 역사 재현율이 60→67% 로 밴드를 벗어났고**(§0.1), 되돌리는 것은 배선이 아니라 **계략·전투 밸런스 조정** — `combat.md` §10 검토 14·16·17 | §5.3~§5.6 |
...
>
> 다만 **발주자 판정이 일곱 건 밀려 있습니다.** 그중 둘이 물량을 직접 가릅니다 —
> **① 「영상」이 사전 렌더 mp4 인가 실시간 3D 인가** (mp4 면 진형·함종에 반응하지
> 못해 관전이 아니라 배경 영상이 됩니다) **② L2 를 켜는가** (S5 10주에 여유가 0 이고
> 실제로는 12~13주로 보입니다).
>
> **①부터 답해 주시겠습니까?**
