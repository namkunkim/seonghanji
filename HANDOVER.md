...
| B4 | 이벤트 연출 텍스트 | 40종 |
| B5 | 막간 시뮬레이션 텍스트 | 5구간 |
| B6 | **표기 개정 일괄 적용** | 23개 문서 (기계 변환 가능) |
...
```
B6 (표기 통일)  →  A2·A3 (스탯 완성)  →  B1 (시작 배치표)  →  A4·A5 (임계·산식)
```

B6를 먼저 하는 이유: **문서가 더 늘어나기 전에 표기를 통일**하는 것이 싸다.
...
> 설계는 사실상 끝났고 남은 것은 수치·물량·표기 통일입니다.
> 권장 순서는 표기 일괄 개정 → 스탯 완성 → 시작 배치표 → 임계·산식입니다.
