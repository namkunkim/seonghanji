...
> **2026-09-13 DEMO-RC-G8-01 — 매 턴 기본 승리 조건 판정 — PASS:** `normal-demo-victory-v1`이 G8-00의 immutable original/current composition 비용과 전대 항복 상태를 매 resolution 직후 소비한다. 유비군 70% 비용 손실·전 전대 항복은 조조군 승리, 조조군 같은 조건은 손유 연합 승리, 손권군만의 패배는 유비군 지속 시 전투 계속이며 같은 턴 양측 종료는 조조군 우선이다. 결과가 있으면 같은 턴 `battle_concluded`로 바뀌어 다음 턴을 거부하고, 결과가 없으면 `victory_check → continue_turn`이 유지된다. 기함·탈출·20턴 비교는 G8-03, 장수 casualty는 계속 후속 경계다. G8-01 19/0, G8-00 core/UI 112/0·35/0, S5-02 UI 34/0, G10 entry 24/0, 전체 core 701/0. 다음은 **G8-02 — 유비군 패배와 손유 연합 판정 관계 검증**이다.

> **2026-09-13 S5-02 — 적벽 실제 3D 전투 증거창 통합 — PASS:** 시작 기준 detached `HEAD`/`origin/main`은 `faa803f454aa4c0fb81087ef9ca9fa8e8ae4a327`였다. 사용자 승인에 따라 실제 Main→적벽 준비→20턴 전투 화면에 기존 2D canonical 명령권을 유지한 채 읽기 전용 3D 증거창을 기본 2:1·확대 1:2·Esc 복귀로 연결했다. 3D 입력은 신규 `viewer_3d_projection()` 하나뿐이며 아군 공개 상태, 공개 지형, opaque 확인/추정 접촉, viewer event와 원장 metadata만 깊은 복사한다. 공개 이동 endpoint를 `movement_resolved`로 정화해 항적에 연결하고 사격·피격·보급·EW·연쇄폭발도 공개 사건 범위에서만 표현한다. `Re_Legend_of_the_Galactic_Heroes`의 사용자 소유 GLB 7종/PBR JPG 21개/배경을 별도 v1 디렉터리에 반입해 manifest·SHA-256·출처를 기록했고, 원본 -X 함수는 model yaw -90도로 런타임 -Z 함수/+Z 후미에 맞췄다. 204척 초과는 stable sampling하며 정적 projection은 변경 시에만 SubViewport를 갱신하고 Compatibility에서는 내부 560×436·MSAA/그림자 폴백을 쓴다. 전체 코어 701/0, save 73/0(+skip 2), campaign replay 29/0, SCN-03 save/replay 40/0, S5 집중 및 Forward+/Compatibility GPU 실패 0, 독립 QA P1/P2 0이다. 캡처와 성능 JSON은 `out/s5-02-red-cliff-visible-fleet-3d/`에 있으며 미커밋이다. 다음은 **DEMO-RC-G8-01 — 매 턴 전체 승리 조건 판정**이고 여기서는 시작하지 않는다.

> **2026-09-12 DEMO-RC-G8-00 — 전투 피해·사기·센서·지형 효과 적용 — PASS:** 시작 기준은 detached `HEAD`와 `origin/main`이 모두 `783489b93e1925118036d667d9238d3fbe43d517`였다. `normal-demo-combat-effects-v1`을 추가해 일반/추정 사격의 결정론적 명중·피해, 누적 hull/composition casualty, 사기와 retreating 잠금, 연쇄 폭발의 최대 hull 40% 피해·사기 충격·다음 턴 2턴 sensor -40%·160×160 위험 지대를 실제 battle state에 원자 적용했다. 같은 턴 공격은 모두 피해 전 snapshot에서 합산하고, 적용 뒤 SHP-05 전대 상태를 동기화한 다음 outbound 보급을 실행한다. 위험 지대는 기존 지형 resolver에 합쳐 이동 15000bp·sensor -15%·concealment +8·range 8500bp·arc -10°를 실제 소비한다. viewer는 아군 exact 상태와 가시 적의 opaque band만 공개하며 sealed target mapping과 raw 적 ID·좌표·수치는 숨긴다. phase ledger는 실제 효과를 기록하고 `victory_inputs.winner_present=false`만 제공한다. winner는 G8-01, 장수 casualty와 실제 퇴각 탈출은 후속 범위다. 독립 QA는 마지막 chain 입력 validator까지 재감사해 P1/P2 0을 확정했다. G8 core 112/0, public-flow UI 35/0, 핵심 G4/G5/G6 17개 1291/0, 집중 합계 1438/0이며 focused SCRIPT ERROR는 0이다. 전체 `tests/run_tests.gd`의 697/2는 기존 sandbox `user://` 저장 실패와 뒤따른 `seed` 접근 오류만 재현한 것으로 G8과 무관하다. 캡처는 `out/demo-rc-g8-00-combat-effects/combat-effects-1600x900.png`, SHA-256 `38DA8C461B91BD92EC8623885848266CCF52EA127CF169FCA4B04BADECA82FA0`; `out/`은 커밋하지 않는다. 상세 계약은 `docs/07-production/demo-rc-g8-00-combat-effects.md`에 있다. 다음은 **DEMO-RC-G8-01 — 매 턴 전체 승리 조건 판정**이며 이번 인계에서는 시작하지 않는다.

> **2026-09-12 DEMO-RC-G6-06 — 보급함 재고·손상 처리량·거점 재적재 — PASS:** 시작 기준은 detached `HEAD`와 `origin/main`이 모두 `8f4fef65`인 clean worktree였다. SHP-05 전대 단위 제한 재고(연료 `10000bp`, 탄약 `12`, 보급 자재 `4`), 정상/중파/대파 처리율 `10000/5000/2500bp`, fractional remainder만 이월하는 턴 credit을 G6-03 자동 보급에 연결했다. 완전 충전만 원자 차감하고 재고 부족 후보는 `waiting_inventory`로 건너뛰며, 파괴는 현재/최대 재고를 생존 비율로 함께 줄이고 나포는 잔여 재고 폐기·captor gain 0·source 비활성화를 recovery 상태와 원자 커밋한다. 같은 세력 거점 한 턴 정박 재적재는 outbound 보급 뒤 실행되어 다음 턴부터 쓴다. G8-00 권위 상태/capture fixture는 판정 완료 전 단계만 허용하고 실제 damage/destroy/boarding trigger는 G8-00에 남겼다. viewer는 자기 provider의 세 재고, 함선 상태, 처리율, 이번 턴 잔여, 정상 기준, 재적재만 읽기 전용으로 공개하며 적 정확 정보와 mutation UI는 없다. clean 기준선에서 이미 활성인 G5-03 terrain 상태가 `pending_neutral`로 남아 validator와 충돌한 한 줄을 `active_normal_demo`로 교정했다. 독립 QA 초기 P1/P2를 모두 닫았고 최종은 core 68/0, UI 26/0, Windows GPU 28/0, G6-01~06 관련 headless 822/0, 전체 코어 701/0, P1/P2 잔여 0이다. 캡처는 `out/demo-rc-g6-06-supply-inventory/supply-inventory-1600x900.png`, SHA-256 `349430A05134E2CCD696F2A4F9EBBAD7A81DF37BC2E6BEE456796C09E0CEDCAA`; `out/`은 커밋하지 않는다. 상세 계약은 `docs/07-production/demo-rc-g6-06-supply-inventory-ui.md`에 있다. 다음은 **DEMO-RC-G8-00 — 전투 피해·사기·센서·지형 효과 적용**이다.

> **2026-09-12 DEMO-RC-G6-05 — 표류·구조·나포와 나포 보급함 무력화 — PASS:** 잔여 연료/10 이동 clamp를 실제 판정과 미리보기에 공통 적용한다. 연료 0으로 아군 보급 영역 밖에 도달하면 명령·사격·임무·귀환을 잠그고 다음 턴부터 마지막 방향으로 40씩 표류한다. 구조 장비+구조 임무 아군·연합 고속정과 적 operational 전대의 반경 40 swept-path 최초 접촉을 자동 판정하며, 구조/나포 결과는 전투 terminal이다. 영역 안 연료 0은 정박 보급 뒤 active로 복귀한다. G8-00 권위 나포 intent는 해당 SHP-05 source를 같은 턴 처리량 0·captor gain 0으로 만들고 queue·귀환·보급에서 제거하지만, 실제 damage/boarding trigger는 G8-00, 재고·손상·거점 재적재는 G6-06에 남긴다. own/confirmed/estimated/hidden 정보 경계와 수동 구조·나포 버튼 0을 유지한다. 독립 QA 후 core 93/0, UI 67/0, GPU 71/0, 관련 headless 952/0, 전체 코어 701/0이다. 캡처는 `out/demo-rc-g6-05-fast-craft-recovery/`의 incident/preview PNG이며 SHA-256은 `FE6A811377889AE0ECF473919B91A1226CB4508209071E1434157929FEB81651` / `B0FCF67BFF22161033B0B04FFDFE558F018609EA921AA35F2A93BB571F0655AF`; `out/`은 커밋하지 않는다. 다음은 **DEMO-RC-G6-06 — 보급함 재고·손상 처리량·거점 재적재**다.

> **2026-09-12 DEMO-RC-G6-04 — 비상 귀환 기준·목적지 재탐색·조기 귀환·UI 경고 — PASS:** 실제 이동 거리 기반 연료 소비와 보급 영역 경계까지의 필요 연료, 유효 속도 1턴 예비 연료로 비상 기준을 계산한다. 임계값 이하 고속정은 발포를 중지하고 귀환 이동/HOLD로 잠기며, 목적지는 매 판정 턴 현재 위치에서 `필요 연료→경계 거리→source ID` 순으로 재탐색한다. 직접 지휘 세력은 임계값 전에 조기 귀환을 요청·취소할 수 있고 보급 완료 시 intent가 해제된다. 도달 불가는 경고만 기록하며 실제 표류·구조·나포는 G6-05 범위다. UI는 자기 세력의 연료·기준·ETA·경로·경고와 실제 소비 이벤트만 표시한다. 독립 QA에서 경계 거리·hold-fire·보급 완료 해제와 `out_of_scope` 정합성을 교정하고 비정수 올림 회귀와 지도 라벨 충돌 회피를 보강했다. 최종 시험은 core 46/0, UI 42/0, GPU 44/0, 관련 headless 761/0, 전체 코어 701/0이다. 캡처는 `out/demo-rc-g6-04-fast-craft-return/fast-craft-return-1600x900.png`, SHA-256 `71ADAAC8192D3D3A59B609DD79078911B4C41E35B3BFE3AF71BD419F98D61274`; `out/`은 커밋하지 않는다. 다음은 **DEMO-RC-G6-05 — 표류·구조·나포와 나포 보급함 무력화**다.
