...
>
> **큐 종료 후 정리(2026-08-23):** 세 건을 닫았다. ① **사기 범위** — 0~100 대 45~125의 어긋남을 **게이지 0~125 · 명목 100**으로 확정하고 「고양」 구간 신설. ② **이릉협도 화공 ×3** — 계수를 낮추지 않고 **연영도**를 신설해 조건부로 전환(V-20). ③ **219 일극형 1.88배** — 배치가 아니라 **검산이 틀렸다.** 유효 세력만 세면 2.60배이며, 두 판정의 모집단이 다르다는 것을 `world-state.md` §4.0-c에 명문화(V-21). **§5에 설계 충돌은 더 남아 있지 않다.**
>
> **문서 간 공백 정리(2026-08-23):** 코드 착수를 실제로 막던 두 건을 닫았다. ④ **전화 회복 계수** — `interludes.md`의 막간 배당이 의존하는 비례 규칙이 `region-power.md`에 없었다. 체감형 `Δ = (0.95 − C) × r`로 신설했고, **상한 0.95는 새로 정한 값이 아니라 §3.1에 이미 있던 「전란을 겪지 않은 권역의 계수」**였다(V-22). ⑤ **계략 성향 계수의 AI 대응** — 계층은 이미 `ai-design.md` §2에 배정되어 있었고, 진짜 공백은 **「누구의 성향인가」**였다. **입안자(주유 ×0.9)이지 실행자(황개 ×0.6)가 아니다**(V-23).
...
| **문서** | **32개** (약 83만 자) + HTML 지도 1개 |
| **코드** | **미착수** |
| **남은 것** | **프로토타입 코드** — 착수 체크리스트는 §4.3 |
...
4. docs/DECISIONS.md    ← 기각된 안과 그 이유 (V-01~V-23)
...
### 4.3 다음 작업 — 프로토타입 코드 [TODO]

**설계는 끝났다. 남은 최대 위험은 그래픽이 아니라 밸런스다** —
`CLAUDE.md` §5 함정 2(「대부분의 수치는 미검증」)가 아직 살아 있다.
제작 요건 전반은 **`docs/07-production/dev-requirements.md`**에 정리되어 있다.

#### P0. 코드 한 줄 전에 정할 것 [사용자 결정 필요]

착수 전 **사람이 답해야** 하는 것들이다. 순서가 있다.

- [ ] **P0-1. 목표 규모** — 본편 완전판(약 320 인월)인가 **축소판**(약 145 인월)인가
      → 그 밖의 모든 것을 막고 있다 (`dev-requirements.md` §10.3)
- [ ] **P0-2. 서버 시간 권위 채택** — 시간을 팔지 않기로 했으므로 **시간을 훔치는 것이
      최대의 부정행위**가 된다. 이 결정이 「완전 오프라인 단독 실행」을 지운다 (§2.1)
- [ ] **P0-3. 데이터 정본화 · ID 체계** — 문서는 근거, `data/*.json`이 정본 (§3)
- [ ] **P0-4. 엔진 선택** — 권고는 Unity 6.x + .NET 서버. 근거는 그래픽이 아니라
      **시뮬레이션 코어 한 벌 공유** (§4.2)
- [ ] **P0-5. 규칙 버전 고정 방식** — 완주가 5.8개월이라 **그 사이 밸런스 패치가 나간다.**
      나중에 붙일 수 없다 (§2.4)
...
- [ ] **P1-1. 데이터 추출** — 문서 → `data/*.json` 11종 (`dev-requirements.md` §3.3)
- [ ] **P1-2. 정합성 검증기** — 참조 무결성 · 총량(권역 45 · 성계 19 · 회랑 15) ·
      범위(사기 0~125 · 천명 0~100) · 도달성 · 미사용 (§3.5)
- [ ] **P1-3. 시뮬레이션 코어** — 엔진 비의존 순수 라이브러리 한 벌.
      **결정론 필수**(부동소수 금지 · 시드 고정 · 순회 순서 고정) (§2.3)
- [ ] **P1-4. 서버측 가상 시계** — 60배~1만 배속. **시간 소유 구조와 동시에 설계**해야 하며
      **상용 빌드에 존재해서는 안 된다** (§8)
- [ ] **P1-5. 헤드리스 시뮬레이터** — `ai-design.md` §11의 검증 도구 다섯

**합격 기준은 이미 숫자로 있다** (`ai-design.md` §11.1)
...
> **시험 기준이 있는데 시험 도구가 없다.** 그래서 시뮬레이터가 먼저다.
> 그리고 이것은 버려지는 코드가 아니라 **본편의 심장**이다 —
> 클라이언트·서버·시뮬레이터가 같은 코어를 참조한다.

#### P2. 측정으로 풀리는 검토 포인트

§5의 미해결 항목 중 다수는 **논쟁이 아니라 측정**의 문제다.
P1이 끝나면 아래를 돌려서 답한다.

- [ ] `combat` §10-13 — 적벽 분기 A **27.4%는 밀집 진형 성립을 전제한 상한**이다. 실제 발생률은?
- [ ] `ai-design` §12-1 — 명문형 AI가 결단 임계로 마비되는 빈도
- [ ] `ai-design` §12-2 — 절단 가치 과최적화로 AI가 단조로워지는가
- [ ] `scenario-228-263` §9-1 — 공세 3회 실플레이의 피로도 (J7이 6→3으로 줄인 뒤)
- [ ] `world-state-setup` §10-7 — 변주형 실동원 정밀 산출 (200 일극형 3.17이 잠정값)
- [ ] **엔딩 분포** — **Division 엔딩이 실제로 도달되는가.** 불가침 원칙 ⑤가 걸려 있다

#### P3. 그 밖의 미착수 영역

- [ ] 기동병기 상세 (에이스 판정)
- [ ] 튜토리얼·온보딩
- [ ] 아트·사운드 방향 — 인물 **492**의 3단 차등 채택 여부 (`dev-requirements.md` §5.1)
- [ ] **복귀 브리핑 서술 템플릿** — 이 게임의 첫 화면인데 생성 방식이 미정 (`ui-design` §9-4)
- [ ] 특수 엔딩 5종의 세력 변주 — 최소 **Extinction 4계열** (`ending-variations` §6-1)

### 4.4 완료 기록 — 설계 큐 J1~J12
...
**설계 큐에 남은 것은 없다. 12개가 전부 닫혔다.** 다음 작업은 위 §4.3이다.
...
| 6 | 수치는 대부분 미검증이다 |
| 7 | **동시 세션이 서로를 덮어쓴다** — 사고가 **양방향으로 두 번** 났다. `git commit`은 인덱스 전체를 커밋하므로 반드시 `git commit -- <경로>`로 경로를 명시한다. 문서 전역 눈금(스케일·범위)은 신설 전에 `docs/INDEX.md`를 확인한다. 상세는 §4.4 각주 |
| 8 | **한 문서가 다른 문서의 미정 수치에 기대어 진행된 사례가 둘 있었다** — 전화 회복 계수(V-22) · 계략 성향 계수(V-23). 둘 다 「대응이 없다」고 적혔지만 **한쪽은 상한이, 한쪽은 계층이 이미 상대 문서에 있었다.** 검토 포인트에 「없다」를 적기 전에 상대 문서를 한 번 더 볼 것 |
...
> 설계 문서는 32개입니다. **다음은 프로토타입 코드**이고, 착수 체크리스트가 §4.3에 있습니다.
>
> 다만 **P0의 다섯 결정은 사람이 답해야 합니다.** 그중 첫째가
> **목표 규모 — 본편 완전판(약 320 인월)인가 축소판(약 145 인월)인가**이고,
> 이 하나가 나머지 전부를 막고 있습니다 (`dev-requirements.md` §10.3).
>
> **P0-1부터 답해 주시겠습니까?**
