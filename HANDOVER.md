...
| **현재** | **D1 기반 안정화** — 첫 착수 묶음 `A-01`(캠페인 저장 모델)·`A-03`(명령 판정 코어 이동)·`A-07`(테스트 신뢰성) **세 슬라이스 origin 반영 (2026-09-02)**. 진행·검증 상태 정본 = `PROJECT-TRACKER.md` §2 D1·§5. 잔여 = `A-02`(저장 지문 인수 7종)·문서 동기화. S3 화면 잔여(S3.5·S3.7·S3.8)는 D1 뒤 D2 묶음 |
...
| 작업 | 무엇 | 계약·요구사항 | 착수분 |
|---|---|---|---|
| **A-01** | 캠페인 단위 저장·불러오기 모델 확정 | `preproduction-save-contract.md` §2(저장 범위 = 시드+명령 로그, 나머지 파생)·§3(경계 스냅숏 + 순수 로그 재생, 손상 복구 A-02 전 로드 거부, 자동 저장 월 정산). 캠페인 재생 경로(`Campaign` 셋업→목표 틱)를 새로 만든다 | 🟨 `a101327`·`354a689` — `schema/save-campaign.json` · `Campaign.digest()`/`from_save()`/`replay_to()` · `Save` origin 필터 · `tests/run_campaign_replay.gd` 29단언. 불러오기 실측: 전장 SCN-3 재생 ≈1.5초 → 순수 로그 재생 유지. **잔여:** 지문 인수 7종 시험 = A-02 · 문서 체크박스 |
...
| **A-07** | 테스트 실행 신뢰성 보강 | `DECISIONS.md` V-61 — 실패 시험이 후속 단언을 숨기지 않음 · 단언 수 하한 664 고정. 시험 5층(단위 / 100회 캠페인 / 검산기 3종 / 저장 복원 / 실기 스모크) | 🟦 `f3fd832`·`27a3509` — `tests/harness.gd`(`EXIT_*`·`MIN_UNIT_ASSERTIONS=664`) · 실패 섹션 격리 · 구조 검증 exit · `run_save_restore.gd` 스텁(케이스 = A-02) |
