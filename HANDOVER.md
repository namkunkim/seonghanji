...
> 갱신일: 2026-08-22 (A2·A3 완료 반영)
>
> **[핸드오버 요약]** A2·A3 완료 — 일반 무장 252인·이역 93인 스탯·성향 규칙 기반 산출. 서방 원수급 19인은 확장 콘텐츠로 유보. 검수 대기, 다음은 B1.
...
| # | 작업 | 규모 | 비고 |
|---|---|---|---|
| ~~A2~~ | ~~일반 무장 256인 스탯 + 성향 확정~~ | **완료 (2026-08-22).** 규칙 기반 산출, 실제 252인. `officers-256.md` §6. **검수 대기** |
| ~~A3~~ | ~~이역 인물 90인 스탯~~ | **완료 (2026-08-22).** 규칙 기반 산출, 실제 93인. `foreign-90-stats.md`. **검수 대기.** 로마·팔미라·사산조 원수급 19인은 확장 콘텐츠로 유보 확정 |
...
```
~~A2·A3 (스탯 완성)~~  →  B1 (시작 배치표)  →  A4·A5 (임계·산식)
```

B6(표기 통일)는 완료되어 순서에서 제외. A2·A3(스탯)도 2026-08-22 완료 — 다음은 **B1(시작 배치표)**부터 이어간다.
단, A2·A3은 **규칙 기반 자동 산출**이라 개별 인물의 서사적 개성까지는 못 담는다 — 두드러진 인물은 검수 단계에서 손으로 다듬을 것.
...
| `generals-stats` §8-2 | 여성 인물 통솔·무력이 대부분 10~30. 편성 다양성 제약 |
