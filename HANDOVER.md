...
| **현재** | **D1 기반 안정화** — 첫 착수 = `A-01`(캠페인 저장 모델)·`A-03`(명령 판정 코어 이동)·`A-07`(테스트 신뢰성). 각 계약은 P0-09 게이트에 명시 (§4.1). S3 화면 잔여(S3.5·S3.7·S3.8)는 D1 뒤 D2 묶음 |
...
| 작업 | 무엇 | 계약·요구사항 |
|---|---|---|
| **A-01** | 캠페인 단위 저장·불러오기 모델 확정 | `preproduction-save-contract.md` §2(저장 범위 = 시드+명령 로그, 나머지 파생)·§3(경계 스냅숏 + 순수 로그 재생, 손상 복구 A-02 전 로드 거부, 자동 저장 월 정산). 캠페인 재생 경로(`Campaign` 셋업→목표 틱)를 새로 만든다 |
| **A-03** | 명령 산정·검증을 코어 권위로 이동 | `DECISIONS.md` V-60 §8.1 — 화면↔코어 갈라짐 5건(진형·함종별 척수·강화 축·관측 단계·이동 판정)의 정본 소유자 = core. 완료 기준 = 「동일 입력에서 UI·AI·재생 결과 일치」 |
| **A-07** | 테스트 실행 신뢰성 보강 | `DECISIONS.md` V-61 — 실패 시험이 후속 단언을 숨기지 않음 · 단언 수 하한 664 고정. 시험 5층(단위 / 100회 캠페인 / 검산기 3종 / 저장 복원 / 실기 스모크) |
