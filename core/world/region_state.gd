...
## 개발 단계. 개발여지를 소비해 생산·수입을 올린다.
## 상한은 권역의 개발여지 칸 수다 (하 1 · 중 2 · 상 3 · 극상 4)
var development: int = 0

## **직할인가 위임인가** (domestic.md §3).
##
## 위임하면 수입 −30% · 행정비 −70% · **실동원 −50%** 이고 명령이 즉시 닿는다.
## 대가의 본체는 실동원이다 — **위임한 땅의 병력을 중앙이 쓰지 못한다.**
## 주목이 어떻게 군벌이 되었는가가 이 한 칸이다.
