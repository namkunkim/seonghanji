...
## **복구가 목록에 없는 노선이 있다.**
##
## `region-power.md` §2.1 이 「**복구는 순수 비용이다**」라고 못 박았다 —
## 자금으로는 회수되지 않고 천명만 오른다. 실제로 계산해도 그렇다:
## 복구 1단계는 월 인구×20 을 쓰고 연 0.5%p 를 되돌린다. **회수에 40년이 걸린다.**
## 시나리오 3 은 3년이다.
##
## 그래서 §2.1 이 이렇게 적었다 —
## 「**실력 노선 플레이어는 같은 자금을 함대에 쓰는 편이 합리적이며, 실제로 그것이 옳다.**」
##
## **AI 도 그것이 옳다.** 정통 노선(인덕·명사·명문)만 복구한다.
##
## ⚠ **징병도 뺐다.** `RegionState.garrison` 은 전투에 쓰이지 않는다
## (`combat.md` 는 함대만 다룬다 — `domestic.md` 미작성 항목).
## **효과가 없는 곳에 AI 가 자금을 넣으면 안 된다.**
## 지상 병력이 전투에 들어오면 되돌린다.
