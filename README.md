...
**성한지: 천명** — 후한 말 삼국시대를 우주 SF로 옮긴 Windows PC 우선·Android/iOS 지원 전략 시뮬레이션.

이 저장소에는 기획 문서와 **참고 프로토타입 코드**가 있습니다 — 기준선은 코어 27파일 · 화면 16파일 ·
시험 35섹션 701단언 · 검산기 3종입니다. 화면 없이 AI 대 AI로 100회 자동 진행되는 M0 시뮬레이터가
2026-08-28에 게이트를 통과했고(`docs/07-production/m0-report.md`), 그 위에 최소 플레이 가능
클라이언트(S3)를 올리는 중입니다 — S3.5 명령 메뉴 · S3.7 알림 배너 · S3.8 저장이 잔여입니다.

> **2026-09-06 검수 기준:** G-10 적벽은 조건 원장·DEC-01·pending battle·participant manifest와
> `pending → active` 전이까지 커밋됐습니다(`675347f`, `7c7c6c5`). 활성 5페이즈 진행·resolved 결과·
> 뉴스 exactly-once·HomeMap 코어 projection은 미구현이므로 G-10 완료가 아닙니다. 로컬 홈 지도·함대
> 이동·3D 항행 변경도 C-01 및 Windows 1600×900 실기 검증 전에는 C-04 완료로 보지 않습니다.
...
| 서사 | 프롤로그 · 엔딩 10종 |
| AI · UI | 설계 초안 |
| **코드** | 참고 프로토타입 — 기준선 코어 27파일 · 화면 16파일 · 시험 35섹션 701단언. M0 게이트 통과(2026-08-28) · G-10과 홈 지도/함대 항행은 부분 구현·검증 중 |
