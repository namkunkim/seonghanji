...
이번 UI는 자동 보급 영역, queue, 한 턴 정박, 중단, 처리 순위·처리량, 연료와 실제 G4 finite refill 완료만 다룬다. 비상·조기 귀환은 G6-04, 표류·구조·나포와 나포 보급함 무력화는 G6-05 전투 UI의 별도 viewer receipt가 담당한다. 보급원 재고·손상·재사용과 기지 재장전은 G6-06 이후, 실제 damage/boarding trigger는 G8-00 경계다. 화면은 `Control` 기반 2D 전용이다.
