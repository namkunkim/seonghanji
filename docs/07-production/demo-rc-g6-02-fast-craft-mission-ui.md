...
이번 기능은 전술 임무의 선택, 초안, 제출 적용, 다음 턴 예약·취소·승격 상태까지만 다룬다. 임무 효과, 연료, 보급, 귀환, 표류, 구조 결과와 나포 결과는 receipt의 `result_pending` 경계대로 G6-03 이후이며, UI가 결과를 계산하거나 성공·피해를 암시하지 않는다. 화면은 `Control` 기반 2D만 사용한다.
...
캡처는 판정 중 Liu 자기 전대의 불변 정찰 장비, 활성 연락 임무, 정찰 다음 턴 예약, 예약 취소 조작, 적용 턴과 G6-03+ 결과 경계를 보여 준다.
