...
이번 기능은 전술 임무의 선택, 초안, 제출 적용, 다음 턴 예약·취소·승격 상태까지만 다룬다. 연료와 실제 G4 finite 자원의 자동 보급은 G6-03, 비상·조기 귀환은 G6-04, 표류·구조·나포는 G6-05 전투 UI가 각각 별도 viewer receipt로 표시한다. 실제 damage/boarding trigger는 G8-00 경계이며, 임무 UI가 결과를 계산하거나 성공·피해를 암시하지 않는다. 화면은 `Control` 기반 2D만 사용한다.
