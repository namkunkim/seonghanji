...
이번 기능은 전술 임무의 선택, 초안, 제출 적용, 다음 턴 예약·취소·승격 상태까지만 다룬다. 연료와 실제 G4 finite 자원의 자동 보급은 G6-03 전투 UI가 별도 viewer receipt로 표시한다. 임무 효과, 자동 귀환, 표류, 구조 결과와 나포 결과는 G6-04 이후이며, 임무 UI가 결과를 계산하거나 성공·피해를 암시하지 않는다. 화면은 `Control` 기반 2D만 사용한다.
