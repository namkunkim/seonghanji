...
- 2D 전술 지도는 명령·선택의 단일 권위로 유지한다. S5-02의 3D 증거창은 viewer-safe 공개 projection만 소비하는 읽기 전용 표현이며 코어 판정·저장 상태·미탐지 적 정보를 소유하지 않는다.
...
비목표는 전투 수치·새 기능 이벤트 수치·새 화공 확률·태양계권 소유 규칙을 정하는 것과 현재 모든 즉시 전투를 지속형으로 교체하는 것이다. 화면·에셋·3D 구현은 2026-09-13 사용자 승인에 따라 S5-02 별도 계약에서 수행하되 이 코어 계약의 권위를 넓히지 않는다. 이 계약은 적벽만 지속형으로 시작하는 안과 전체 전투를 일반화하는 안 중 하나를 확정하지 않는다.
