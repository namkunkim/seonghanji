...
후속 피해·사기·센서 장애·지형 효과는 계산하거나 확정하지 않는다. 화면에는 `후속 효과 판정 대기`, `일반 승리 판정 대기`만 표시한다. 런타임 표현은 Control 기반 2D이며 Node3D, GLB, 영상 자산을 사용하지 않는다.
...
실제 피해, 사기 충격, 센서 장애, 임시 지형 위험, 사상자, 승패 판정은 이 작업에서 구현하거나 표시하지 않는다. 작전 전용 스틸 이미지나 음향 연출도 현재 범위에는 포함하지 않았다.
