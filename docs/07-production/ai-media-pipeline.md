...
>
> **단계별 절차(계정 → 볼륨 → 파드 → 가중치 → 시험생성 → 배치 → 후처리 → 반출)는
> `runpod-runbook.md` 로 분리했다.** 이 절은 결정(무엇을·왜), 런북은 실행이다.
...
> **이 절은 BGM 6트랙의 생성 규격(길이·조성·BPM·루프 방식·프롬프트·파라미터)의 정본이다.**
> **씬↔큐 매핑 · 크로스페이드 규칙 · 믹스 타깃(LUFS) · 효과음(SFX) 목록 · 혼합 출처
> 정규화 · R-14 검수 순서 · SFX 권리 규율은 `preproduction-audio-plan.md`(P0-05)로 분리했다.**
> 이 절을 고치면 그 문서 §1(요약 참조)도 함께 본다.
...
> ⚠ `[미확정]` = 아직 생성·반입되지 않은 자산. 현재 확정 에셋과 계획 물량을 섞지 않는다.
...
■ AI 생성물 안내
  ...
  이 게임에는 생성형 AI 도구로 제작한 에셋이 포함될 수 있습니다.
  현재 확정: 공용 인물 초상 11점.
  제작 완료 시 반영: 명장 초상 최대 120점 · 배경음악 6곡.

■ 사용 모델

  인물 초상 — FLUX.1 [schnell] — Apache License 2.0 — Black Forest Labs
  배경음악 — ACE-Step 1.5 — Apache License 2.0 — ACE Studio · StepFun

  생성 실행 — RunPod (클라우드 GPU, 선택 표기).
...
- ~~이미지 모델 최종 택1~~ → **V-50: FLUX.1 [schnell] 확정, SDXL 신규 인물 제작 중단**
...
- 배경 `ART-R101~104` 는 조건부다. L3 컷씬은 V-52로 채택됐으나 `VID-002`는 D4까지
  보류다. D6-2는 V-54로 **Runway 1차 검증 → 부적합 시 로컬 Wan 2.2** 순서를 지정했다.
  실제 영상 파일 반입 뒤 ①에 행을 늘리며, 모델·라이선스 문자열은 S6.1 검증 결과만 사용한다.
...
- [x] ~~**초상 규격** (해상도·종횡비·배경·형식)~~ — 2026-08-30. §4.1
- [x] ~~**`ART-C001..120 ↔ CHR-####` 매핑표**~~ — 2026-08-30. §4.7 · `data/portrait-map.json`(생성물 120행·시드)
- [x] ~~**④ 개별부 — `traits` → 시각 요소 변환 규칙**~~ — 2026-08-30. §4.8
- [x] ~~권역 배경 물량 재산정~~ — V-45 로 폐기
- [x] ~~클라우드 GPU 임대처 선정·약관~~ — RunPod Community. 2026-08-30. §3.3 · V-46
- [x] ~~RunPod ToS 상업 이용 조항~~ — 2026-08-30. §3.3-0 (§10은 "the Site" 대상, 컴퓨트는 §4.1 판매 대상)
- [x] ~~`ui-design.md` §8.1 ↔ §8 상호 참조~~ — 2026-08-30
- [~] **크레딧 화면 문안** — 초안 §8 (2026-08-30). 생성 규칙 + 대장 기준 렌더. 확정 잔여는 아래 5

### 남은 것 — 전부 실행 패스(RunPod / 로컬) 이후

- [x] **1. 화풍 기준선 확정** — V-50: v2·FLUX.1 [schnell]·공용 11종 승인, 모델/워크플로/파일 SHA 잠금. 320px UI 연결 검수 통과
- [~] **2. 초상 생성** — 공용 11종 완료·검증. **명장 120종은 별도 발주자 승인 전 자동 착수 금지**
...
- [x] **5. 게임 내 크레딧 문안 확정** (§8.4) — V-53 승인. 게임 내 문안과 V-52 스토어 고지는 `preproduction-license-check.md` §3에 분리. 조건부 ACE-Step·영상·비-CC0 행은 실제 반입·검증 뒤 추가하고 출시 직전 재생성
- [ ] **6. 배경·컷씬 (S6)** — `SC-L2` 배경 슬롯 결정 시 `ART-R101~104`. L3 컷씬은
  V-52 채택(기본 `VID-001·003·004`, `VID-002`는 D4 보류), V-54로 Runway 우선 검증·
  Wan 2.2 폴백 지정. S6.1에서 공식 약관·모델 라이선스를 검증한다. 이 세션 범위 밖
