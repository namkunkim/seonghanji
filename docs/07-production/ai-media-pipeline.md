...
- 배경 `ART-R101~104` 는 조건부다. L3 컷씬은 V-52로 채택됐으나 `VID-002`는 D4까지
  보류이고, 영상 서비스는 D6-2 미지정이다. 실제 영상 파일 반입 뒤 ①에 행을 늘리며,
  모델·라이선스 문자열은 지정 뒤 S6.1 원문 검증 결과만 사용한다.
...
- [ ] **6. 배경·컷씬 (S6)** — `SC-L2` 배경 슬롯 결정 시 `ART-R101~104`. L3 컷씬은
  V-52 채택(기본 `VID-001·003·004`, `VID-002`는 D4 보류)이나 서비스는 D6-2 미지정.
  후보 지정 뒤 S6.1에서 공식 약관·모델 라이선스를 검증한다. 이 세션 범위 밖
